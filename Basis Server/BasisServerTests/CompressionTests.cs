...
namespace BasisServerTests
{
    ...
    public class AvatarDeltaCompressionTests
    {
        [Fact]
        public void EncodeDelta_IdenticalPayloads_ReturnsOnlyBitmask()
        {
            byte[] current = { 1, 2, 3, 4, 5, 6, 7, 8 };
            byte[] baseline = { 1, 2, 3, 4, 5, 6, 7, 8 };
            byte[] delta = new byte[AvatarDeltaCompression.MaxDeltaSize(current.Length)];

            int written = AvatarDeltaCompression.EncodeDelta(current, baseline, delta);

            Assert.Equal(AvatarDeltaCompression.BitmaskSize, written); // only bitmask, no chunks
            // Bitmask should be zero
            uint bitmask = BitConverter.ToUInt32(delta, 0);
            Assert.Equal(0u, bitmask);
        }

        [Fact]
        public void EncodeDelta_AllDifferent_WritesAllChunks()
        {
            byte[] current = { 10, 20, 30, 40, 50, 60, 70, 80 };
            byte[] baseline = { 1, 2, 3, 4, 5, 6, 7, 8 };
            byte[] delta = new byte[AvatarDeltaCompression.MaxDeltaSize(current.Length)];

            int written = AvatarDeltaCompression.EncodeDelta(current, baseline, delta);

            // 1 chunk of 8 bytes + 4 byte bitmask = 12
            Assert.Equal(12, written);
            uint bitmask = BitConverter.ToUInt32(delta, 0);
            Assert.Equal(1u, bitmask); // bit 0 set
        }

        [Fact]
        public void EncodeDecodeDelta_RoundTrip_RestoresOriginal()
        {
            ...
            byte[] output = new byte[baseline.Length];
            AvatarDeltaCompression.DecodeDelta(delta, 0, baseline, output);

            Assert.Equal(current, output);
        }

        [Fact]
        public void EncodeDecodeDelta_PartialLastChunk_HandledCorrectly()
        {
            // 10 bytes = 1 full chunk (8) + 1 partial chunk (2)
            byte[] baseline = new byte[10];
            byte[] current = new byte[10];
            current[9] = 99; // change only in partial chunk
        ...
        [Fact]
        public void MaxDeltaSize_CorrectCalculation()
        {
            // 8 bytes = 1 chunk => 4 + 1*8 = 12
            Assert.Equal(12, AvatarDeltaCompression.MaxDeltaSize(8));
            // 16 bytes = 2 chunks => 4 + 2*8 = 20
            Assert.Equal(20, AvatarDeltaCompression.MaxDeltaSize(16));
            // 10 bytes = 2 chunks => 4 + 2*8 = 20
            Assert.Equal(20, AvatarDeltaCompression.MaxDeltaSize(10));
            // 1 byte = 1 chunk => 4 + 1*8 = 12
            Assert.Equal(12, AvatarDeltaCompression.MaxDeltaSize(1));
        }

        [Fact]
        public void EncodeDecodeDelta_OnlySecondChunkChanged()
        {
            byte[] baseline = new byte[16];
            byte[] current = new byte[16];
            Array.Copy(baseline, current, 16);
            current[8] = 0xFF; // change only in second chunk

            byte[] delta = new byte[AvatarDeltaCompression.MaxDeltaSize(current.Length)];
            int written = AvatarDeltaCompression.EncodeDelta(current, baseline, delta);

            // Should only write bitmask (4) + one chunk (8) = 12
            Assert.Equal(12, written);
            uint bitmask = BitConverter.ToUInt32(delta, 0);
            Assert.Equal(2u, bitmask); // bit 1 set (second chunk)
        ...
        [Fact]
        public void DecodeDelta_WithNetDataReader_Works()
        {
            byte[] baseline = new byte[24];
            byte[] current = new byte[24];
            ...
            // Create a reader from the delta data
            var writer = new Basis.Network.Core.NetDataWriter();
            writer.Put(delta, 0, written);
            var reader = new Basis.Network.Core.NetDataReader(writer.CopyData());

            byte[] output = new byte[baseline.Length];
            AvatarDeltaCompression.DecodeDelta(reader, baseline, output);

            Assert.Equal(current, output);
        }

        [Fact]
        public void EncodeDelta_XorEncoding_IsCorrect()
        {
            byte[] baseline = { 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22 };
            byte[] current = { 0x55, 0x44, 0x33, 0x22, 0x11, 0x00, 0xEE, 0xDD };
            byte[] delta = new byte[AvatarDeltaCompression.MaxDeltaSize(current.Length)];

            AvatarDeltaCompression.EncodeDelta(current, baseline, delta);

            // Check XOR values start at offset 4 (after bitmask)
            for (int i = 0; i < 8; i++)
            {
                Assert.Equal((byte)(current[i] ^ baseline[i]), delta[4 + i]);
            }
        }
    }
