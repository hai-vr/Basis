...
/// </summary>
public class BasisBeeAvatarPreviewWindow : EditorWindow
{
    private const string ScenePreviewPrefix = "Generic Avatar Preview (";

    [MenuItem("Basis/Build/BEE Avatar Preview")]
    public static void ShowWindow()
    {
        BasisBeeAvatarPreviewWindow window = GetWindow<BasisBeeAvatarPreviewWindow>("BEE Avatar Preview");
    ...
    {
        EditorGUILayout.LabelField("BEE Avatar Preview", EditorStyles.boldLabel);
        EditorGUILayout.Space(4);
    ...
    private void DrawImposterTab()
    {
        ...
        EditorGUILayout.LabelField("Drag to orbit, scroll to zoom. Rest pose.", EditorStyles.miniLabel);
    ...
    private async Task BuildGenericAvatarAsync(byte[] encryptedSection, BasisProgressReport report)
    {
        ...
        if (!instantiated)
        {
            ...
            DestroyImmediate(genericTemplate);
    ...
    private void DrawGenericTab()
    {
        ...
        {
            EditorGUILayout.HelpBox(string.IsNullOrEmpty(genericSectionInfo) ? "No generic avatar loaded." : genericSectionInfo, MessageType.Info);
            return;
        }
        ...
        if (Event.current.type == EventType.Repaint)
        {
            if (genericPreview == null)
            {
                ...
                GameObject previewClone = Instantiate(genericTemplate);
        ...
        EditorGUILayout.LabelField($"{renderers.Length} renderers. {genericSectionInfo}", EditorStyles.miniLabel);
        EditorGUILayout.LabelField("Drag to orbit, scroll to zoom. Rest pose, glTF PBR materials.", EditorStyles.miniLabel);
    ...
    private void SpawnGenericInScene()
    {
        ...
        GameObject spawned = Instantiate(genericTemplate);
    ...
    private void DrawInfoTab()
    {
        ...
        EditorGUILayout.Space(4);
        EditorGUILayout.LabelField("Generic (glTF) Avatar", EditorStyles.boldLabel);
        if (genericTemplate == null)
        {
        ...
        if (genericData == null)
        {
            EditorGUILayout.HelpBox("Section has no BasisGenericAvatarData — a client cannot rebuild a humanoid from it.", MessageType.Warning);
            return;
        }
        ...
        if (!string.IsNullOrEmpty(humanoidTestResult))
        {
            ...
            EditorGUILayout.HelpBox(humanoidTestResult, passed ? MessageType.Info : MessageType.Error);
        }
    }
    ...
    private string RunHumanoidRebuildTest()
    {
        ...
        try
        {
            testClone = Instantiate(genericTemplate);
    ...
    {
        float side = Mathf.Clamp(position.width - 24f, 200f, 480f);
    ...
    private void ReleaseLoaded()
    {
        ...
        if (farLodMesh != null)
        {
            DestroyImmediate(farLodMesh);
            farLodMesh = null;
        }
        if (farLodTexture != null)
        {
            DestroyImmediate(farLodTexture);
            farLodTexture = null;
        }
        if (farLodMaterial != null)
        {
            DestroyImmediate(farLodMaterial);
            farLodMaterial = null;
        }
