...
{
    // Basis ▸ Debug ▸ IK ▸ Foot IK Sweep.
    public class BasisFootIKSweepWindow : EditorWindow
    {
        ...
        [MenuItem("Basis/Debug/IK/Foot IK Sweep")]
        public static void ShowWindow()
        {
            var w = GetWindow<BasisFootIKSweepWindow>("Foot IK Sweep");
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.LabelField("Avatar (T-pose measurements)", EditorStyles.boldLabel);
            ...
            EditorGUILayout.LabelField("Key Tunables", EditorStyles.boldLabel);
            ...
            if (GUILayout.Button("Reset to persistentDataPath")) _path = BasisFootIKSweep.DefaultPath();

            EditorGUILayout.Space();
            if (GUILayout.Button("Run Advanced Simulation", GUILayout.Height(32)))
            {
        ...
        void DrawResults()
        {
            ...
            GUI.color = prev;
            EditorGUILayout.HelpBox(gate.reason, gate.pass ? MessageType.None : MessageType.Warning);
            ...
            EditorGUILayout.Space();
            EditorGUILayout.LabelField($"Per-scenario report ({_last.Scenarios} scenarios) — full detail in {System.IO.Path.GetFileName(_last.ReportPath)}", EditorStyles.boldLabel);
            ...
            using (new EditorGUI.DisabledScope(string.IsNullOrEmpty(_last.ReportPath)))
                if (GUILayout.Button("Reveal Report")) EditorUtility.RevealInFinder(_last.ReportPath);
