...
namespace Basis.Network.Core
{
    ...
    public static class BasisConfigXmlDocs
    {
        ...
        private static void RegisterServerConfig()
        {
            ...
            t.Fields.Add(new FieldDoc("BSRMaxDegreeOfParallelism", " Worker cap for the Server Reduction System's parallel phases (send loop, message processing, distance sweep). int; 0 = one worker per CPU core, values above the core count are clamped. The tick runs hundreds of times a second, so every worker costs dispatch and GC-poll traffic per tick; once the per-tick slice is large enough to keep them busy, extra workers cost more than they return. Lower it if the box is shared with other services or if profiling shows time in thread dispatch rather than the send loop. "));
        ...
        private static void RegisterLnlConfig()
        {
            ...
            t.Fields.Add(new FieldDoc("MaxUnreliableQueuePerPeer", " Maximum unreliable packets queued per peer before the oldest are dropped. int; 0 = unbounded. This is the backstop that keeps an overloaded server alive: unbounded, a server that cannot drain its send queue grows the backlog instead of shedding, and at 2000 players that backlog reached ~40 GB before every peer timed out. Bounded, the same overload costs dropped position updates and everyone stays connected. Oldest are dropped first because a newer position update supersedes them. Raise it if players on slow links see stutter under normal load; lower it to shed sooner. "));
            t.Fields.Add(new FieldDoc("PeerUpdateParallelism"," Worker cap for the transport's per-peer update pass. int; 0 = a quarter of the CPU cores, floored at 4 and capped at 8, values above the core count are clamped. The pass runs hundreds of times a second and does little work per peer, so letting it spread across every core costs more in thread wake-up and GC-poll traffic than it saves. Raise it only if profiling shows this pass itself saturating its workers. "));
