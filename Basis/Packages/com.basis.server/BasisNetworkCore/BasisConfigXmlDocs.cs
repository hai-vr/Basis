...
namespace Basis.Network.Core
{
    ...
    public static class BasisConfigXmlDocs
    {
        ...
        private static void RegisterServerConfig()
        {
            ...
            t.Fields.Add(new FieldDoc("EnableUplinkAvatarStream", " Accept client-to-server avatar stream frames and advertise support: a continuous predictive stream with a Gray-code bit-plane sweep that lets the server re-converge after packet loss on its own, removing the periodic uplink keyframe entirely (a further ~15-30% less ingress than deltas, at a flat per-frame cost instead of a spike every half second). Requires EnableUplinkAvatarDelta. false = clients fall back to keyframe+delta. true|false; default true. "));
