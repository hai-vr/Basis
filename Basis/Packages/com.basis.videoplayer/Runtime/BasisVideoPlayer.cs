...
//   (BasisVideoPlayer.OutputTexture); consumers attach a BasisVideoMaterialOutput
...
//
// Typical wiring:
//   var player = gameObject.AddComponent<BasisVideoPlayer>();
...
public sealed class BasisVideoPlayer : MonoBehaviour
{
    ...
    [Header("Identity")]
    [Tooltip("Friendly name shown in the Video Players menu and diagnostics. Defaults to the GameObject name when empty.")]
    ...
    [Header("Audio")]
    [Tooltip("Where decoded audio is routed. UnityAudioSource hands frames to a BasisVideoPlayerAudio on this GameObject. Currently the only supported routing; the property exists so additional routings can be added without breaking call sites.")]
    public BasisAudioRouting AudioRouting = BasisAudioRouting.UnityAudioSource;
    [Tooltip("Linear volume 0..1 applied to the audio path. Maps to BasisVideoPlayerAudio.VolumeGain.")]
    ...
    public IBasisFrameSource Source
    {
        ...
        set
        {
            ...
            DetachSource();
            try { source?.Dispose(); } catch (Exception ex) { BasisDebug.LogError($"BasisVideoPlayer source dispose failed: {ex.Message}", BasisDebug.LogTag.Video); }
            source = value;
    ...
    private void Awake()
    {
        // If a BasisVideoPlayerAudio lives on the same GameObject, it becomes
        ...
        BasisVideoPlayerRegistry.Add(this);
    }
    ...
    public void LoadUrl(string url)
    {
        ...
        {
            BasisDebug.LogWarning("BasisVideoPlayer.LoadUrl called with empty URL.", BasisDebug.LogTag.Video);
            return;
        }
    ...
    public void LoadLocalPath(string path)
    {
        ...
        {
            BasisDebug.LogWarning("BasisVideoPlayer.LoadLocalPath called with empty path.", BasisDebug.LogTag.Video);
            return;
        }
    ...
    private void StartNativeEngineForSource(BasisMediaSource media)
    {
        ...
        try
        {
            ...
            if (!BasisVideoPlayerSecurity.IsUrlAllowed(media.Uri, out string blockReason))
                throw new UnauthorizedAccessException($"BasisVideoPlayer refused to load '{media.Uri}': {blockReason}");
    ...
    public void Play()
    {
        ...
        if (source == null)
        {
            BasisDebug.LogWarning("BasisVideoPlayer.Play called with no Source assigned.", BasisDebug.LogTag.Video);
            return;
        }
        Clock.Reset();
    ...
    public void Seek(TimeSpan position)
    {
        if (seekableSource == null)
        {
            throw new NotSupportedException(
                "BasisVideoPlayer.Seek requires a source that implements IBasisSeekableFrameSource. " +
    ...
    {
        BasisVideoPlayerRegistry.Remove(this);
        TeardownNativeEngine();
        DetachSource();
    ...
    private void TeardownNativeEngine()
    {
        ...
        catch (Exception ex) { BasisDebug.LogError($"BasisVideoPlayer native engine dispose failed: {ex.Message}", BasisDebug.LogTag.Video); }
        nativeEngine = null;
    ...
    private void HandleVideoFrame(BasisVideoFrame frame)
    {
        ...
        int cap = MaxQueueLength > BasisVideoPlayerSecurity.MaxQueueLengthCap ? BasisVideoPlayerSecurity.MaxQueueLengthCap : MaxQueueLength;
        if (videoQueue.Count >= cap)
        {
            if (OverflowPolicy == QueueOverflowPolicy.DropNewest)
            {
                ...
                frame.Data = null;
                if (VerboseLogging) BasisDebug.Log("BasisVideoPlayer dropped newest frame; queue full.", BasisDebug.LogTag.Video);
                return;
            }
    ...
    {
        BasisDebug.LogError($"BasisVideoPlayer source error: {ex.Message}", BasisDebug.LogTag.Video);
        pendingError = ex;
    }

    private void HandleEndOfStream()
    {
        if (VerboseLogging) BasisDebug.Log("BasisVideoPlayer reached end of stream.", BasisDebug.LogTag.Video);
    ...
    private void Update()
    {
        ...
        if (toPresent != null)
        {
            if (LateFrameSkipUs > 0 && now - toPresent.PresentationTimeUs > LateFrameSkipUs)
            {
                ...
                BasisVideoFrameBufferPool.Return(toPresent.Data);
                toPresent.Data = null;
                if (VerboseLogging) BasisDebug.Log($"BasisVideoPlayer skipped late frame ({(now - toPresent.PresentationTimeUs)} us behind).", BasisDebug.LogTag.Video);
                return;
            }
    ...
    private void HandleRestartTimer()
    {
        ...
        if (nativeEngine != null && activeMediaSource != null)
        {
            ...
            if (nativeEngine.IsRunning && st == BasisMediaEngineState.Playing && framesFlowing)
            {
                restartScheduled = false;
                restartRechecks = 0;
                if (VerboseLogging) BasisDebug.Log("BasisVideoPlayer restart skipped; stream recovered on its own.", BasisDebug.LogTag.Video);
                return;
            }
            ...
            if ((st == BasisMediaEngineState.Connecting || st == BasisMediaEngineState.Buffering) && restartRechecks < RestartMaxRechecks)
            {
                ...
                if (VerboseLogging) BasisDebug.Log($"BasisVideoPlayer restart deferred; engine still {st} ({restartRechecks}/{RestartMaxRechecks}).", BasisDebug.LogTag.Video);
                return;
            }

            restartScheduled = false;
            restartRechecks = 0;
            if (VerboseLogging) BasisDebug.Log("BasisVideoPlayer auto-restarting source (stream did not recover).", BasisDebug.LogTag.Video);
        ...
        restartScheduled = false;
        if (VerboseLogging) BasisDebug.Log("BasisVideoPlayer auto-restarting source.", BasisDebug.LogTag.Video);
        if (seekableSource != null && seekableSource.IsPrepared)
        {
    ...
    private void PresentFrame(BasisVideoFrame frame)
    {
        ...
        {
            BasisDebug.LogWarning($"BasisVideoPlayer renderer does not support frame format {frame.Format}; frame dropped.", BasisDebug.LogTag.Video);
            runtimeFormatErrors++;
            BasisVideoFrameBufferPool.Return(frame.Data);
            frame.Data = null;
            return;
