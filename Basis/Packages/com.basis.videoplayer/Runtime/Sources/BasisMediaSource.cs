...
// BasisVideoPlayer.LoadSource. The player hands every network URL to the OS-codec
...
// entered only by assigning BasisVideoPlayer.Source directly. Disallowed schemes
...
public sealed class BasisMediaSource
{
    ...
    // Per-source loop flag. Overridden by BasisVideoPlayer.Loop when assigned;
    // BasisVideoPlayer.Loop is the runtime-mutable knob.
    public bool Loop;

    // Initial playback rate (1.0 = real-time). Mirrors BasisVideoPlayer.PlaybackRate.
    public float PlaybackRate = 1f;

    // Initial volume (0..1). Mirrors BasisVideoPlayer.Volume.
    public float Volume = 1f;

    // Initial mute state. Mirrors BasisVideoPlayer.Mute.
    public bool Mute;
