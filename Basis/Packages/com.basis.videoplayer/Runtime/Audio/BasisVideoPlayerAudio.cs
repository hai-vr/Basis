...
// Wire it to BasisVideoPlayer by subscribing Source.OnAudioFrame -> Enqueue.
// The callback runs on Unity's audio thread; Enqueue may be called from any thread.
public sealed class BasisVideoPlayerAudio : MonoBehaviour, IBasisMediaClockSource
{
    [Header("Output")]
    ...
    public string ClipName = "BasisVideoPlayerAudio";
    ...
    // by BasisVideoPlayer when a BasisNativeVideoSource becomes the backend.
    ...
    public void Enqueue(BasisAudioFrame frame)
    {
        ...
        if (cap <= 0 || cap > BasisVideoPlayerSecurity.MaxQueuedAudioFramesCap) cap = BasisVideoPlayerSecurity.MaxQueuedAudioFramesCap;
    ...
    // arriving audio frame re-anchors this clock source. Called by BasisVideoPlayer
    ...
    private void Awake()
    {
        ...
        {
            BasisDebug.LogError(
                "BasisVideoPlayerAudio: no AudioSource assigned and none found on this GameObject. Assign TargetAudioSource or add an AudioSource component.",
    ...
    private void BuildClipForCurrentFormat()
    {
        ...
        float clipLen = Mathf.Min(ClipLengthSeconds, BasisVideoPlayerSecurity.ClipLengthSecondsCap);
        int lengthSamples = Mathf.Max(rate, Mathf.RoundToInt(rate * clipLen));
        clip = AudioClip.Create(
            name: string.IsNullOrEmpty(ClipName) ? "BasisVideoPlayerAudio" : ClipName,
