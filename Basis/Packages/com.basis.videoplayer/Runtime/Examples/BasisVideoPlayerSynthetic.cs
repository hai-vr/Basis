...
// Drop on a GameObject that has a BasisVideoPlayer. Add a BasisVideoMaterialOutput
...
public sealed class BasisVideoPlayerSynthetic : MonoBehaviour
{
    [Header("Resolution")]
    ...
    [Tooltip("If >0, the source raises OnEndOfStream after producing this many frames. Combine with BasisVideoPlayer.Loop to test restart paths.")]
    [Min(0)] public int EmitEndOfStreamAfterFrames = 0;

    private void Start()
    {
        if (!TryGetComponent(out BasisVideoPlayer player)) return;
