...
Or drop the `Prefabs/VideoPlayerStreaming` prefab in a scene and set the URL on
`BasisVideoPlayerStreaming` (it can auto-pick RTSPT on PC / MPEG-TS on Quest).
Add a `BasisVideoPlayerAudio` (+ `AudioSource`) for sound;
`BasisVideoPlayerNetworking` syncs URL/state across the room.
