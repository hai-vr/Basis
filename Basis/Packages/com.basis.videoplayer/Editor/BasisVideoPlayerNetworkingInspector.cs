...
[CustomEditor(typeof(BasisVideoPlayerNetworking))]
public class BasisVideoPlayerNetworkingInspector : Editor
{
    private const string UxmlPath = "Packages/com.basis.videoplayer/Editor/StyleSheets/VideoPlayerNetworkingSDK.uxml";
    private const string UssPath = "Packages/com.basis.videoplayer/Editor/StyleSheets/VideoPlayerSDK.uss";

    private BasisVideoPlayerNetworking _target;
    private VisualElement _root;
    private Label _canControl, _hasPlayer, _editHint;

    public override VisualElement CreateInspectorGUI()
    {
        _target = (BasisVideoPlayerNetworking)target;
        ...
        if (tree == null)
        {
            _root.Add(new HelpBox("VideoPlayerNetworkingSDK.uxml missing.", HelpBoxMessageType.Error));
            return _root;
        }
    ...
    private bool PlayModeOnly()
    {
        ...
        Debug.LogWarning("BasisVideoPlayerNetworking actions only run in Play Mode.");
        return false;
    }

    private void RefreshStatus()
    {
        if (_target == null) _target = (BasisVideoPlayerNetworking)target;
        ...
        SetPill(_hasPlayer, _target.VideoPlayer != null, live);
    }
