...
namespace Basis.EventDriver
{
	...
	public partial class BasisEventDriver : MonoBehaviour
	{
		...
		private static readonly object EventCallbackLock = new object();
		private static Action _onUpdate;
		private static Action _onLateUpdate;
		private static volatile Delegate[] _onUpdateHandlers = Array.Empty<Delegate>();
		private static volatile Delegate[] _onLateUpdateHandlers =
		...
		public static event Action OnUpdate
		{
			add
			{
				lock (EventCallbackLock)
				{
					_onUpdate += value;
					_onUpdateHandlers =
						_onUpdate?.GetInvocationList() ?? Array.Empty<Delegate>();
				}
			}
			remove
			{
		...
		public static event Action OnLateUpdate
		{
			add
			{
				...
				{
					_onLateUpdate += value;
					_onLateUpdateHandlers =
						_onLateUpdate?.GetInvocationList() ?? Array.Empty<Delegate>();
				}
			}
			remove
			{
		...
		private static void ResetEventCallbacks()
		{
			lock (EventCallbackLock)
			{
				_onUpdate = null;
				_onLateUpdate = null;
				_onUpdateHandlers = Array.Empty<Delegate>();
				_onLateUpdateHandlers = Array.Empty<Delegate>();
			}
		}

		private static void InvokeEventCallbacks(
			Delegate[] callbacks,
			string callbackName
		)
		{
			...
			{
				Delegate callback = callbacks[index];
				try
				{
		...
		public void Update()
		{
			...
			{
				BasisDebug.LogErrorOnce(
					$"BasisEventDriver.Update failed: {ex}",
					BasisDebug.LogTag.Event
				);
			}
		}
		...
		private void UpdateBody()
		{
			...
			while (
				BasisDeviceManagement.mainThreadActions.TryDequeue(
					out System.Action action
				)
			)
			{
				try
			...
			BasisNetworkHandleRemoval.ProcessLifecycleQueue(
				BasisNetworkHandleRemoval.LifecycleBudgetPerFrame
			);
			if (!IsHeadlessClient)
			{
				InputSystem.Update();
			...
			InvokeEventCallbacks(_onUpdateHandlers, nameof(OnUpdate));
		...
		public void FixedUpdate()
		{
			...
			{
				BasisDebug.LogErrorOnce(
					$"BasisEventDriver.FixedUpdate failed: {ex}",
					BasisDebug.LogTag.Event
				);
			}
		}
		...
		public void LateUpdate()
		{
			...
			{
				BasisDebug.LogErrorOnce(
					$"BasisEventDriver.LateUpdate failed: {ex}",
					BasisDebug.LogTag.Event
				);
			}
		}
		...
		private void LateUpdateBody()
		{
			...
			if (BasisLocalPlayer.PlayerReady)
			{
				BasisLocalCameraDriver LocalCameraDriver =
					BasisLocalCameraDriver.Instance;
			...
			{
				BasisDebug.LogErrorOnce(
					$"HVRBasisBuiltInAddresses.Simulate failed: {ex}",
					BasisDebug.LogTag.Event
				);
			}
			...
			bool drawJiggle =
				SMModuleDebugOptions.UseGizmos && SMModuleDebugOptions.UseJiggleVisuals;
			if (drawJiggle)
			{
				JigglePhysics.ScheduleRender();
				JigglePhysics.CompleteRender(
					proceduralMaterial,
					sphereMesh,
					capsuleMesh
				);
			...
			InvokeEventCallbacks(_onLateUpdateHandlers, nameof(OnLateUpdate));
		...
		private void OnBeforeRender()
		{
			...
			{
				BasisDebug.LogErrorOnce(
					$"BasisEventDriver.OnBeforeRender failed: {ex}",
					BasisDebug.LogTag.Event
				);
			}
		}
		...
		private void OnBeforeRenderBody()
		{
			...
			if (BasisLocalPlayer.PlayerReady)
			{
				...
				BasisLocalCameraDriver.Instance.microphoneIconDriver.Simulate(
					DeltaTime
				);
#endif
			}
		...
		partial void ProfileBegin(int section)
		{
			...
			switch (section)
			{
				case PROF_REMOTE_AUDIO_SIMULATE:
					BasisEventDriverProfilerData.RemoteAudioDriverCount =
						BasisRemoteAudioDriver.DriversCount;
					break;
		...
		partial void ProfileEnd(int section)
		{
			...
			switch (section)
			{
				case PROF_NETWORK_APPLY:
					BasisEventDriverProfilerData.NetworkApplyMs = ms;
					break;
				case PROF_DEVICE_MANAGEMENT:
					BasisEventDriverProfilerData.DeviceManagementMs = ms;
					break;
				case PROF_REMOTE_AUDIO_SIMULATE:
					BasisEventDriverProfilerData.RemoteAudioSimulateMs = ms;
					break;
				case PROF_NAMEPLATE_SCHEDULE:
					BasisEventDriverProfilerData.NamePlateScheduleMs = ms;
					break;
				case PROF_BTWEEN:
					BasisEventDriverProfilerData.BTweenMs = ms;
					break;
				case PROF_LOCAL_PLAYER:
					BasisEventDriverProfilerData.LocalPlayerMs = ms;
					break;
				case PROF_REMOTE_FACE_SIMULATE:
					BasisEventDriverProfilerData.RemoteFaceSimulateMs = ms;
					BasisEventDriverProfilerData.RemoteFace_Count =
						BasisRemoteFaceManagement.count;
					break;
				case PROF_REMOTE_AUDIO_APPLY:
					BasisEventDriverProfilerData.RemoteAudioApplyMs = ms;
					break;
				case PROF_JIGGLE_SCHEDULE:
					BasisEventDriverProfilerData.JiggleScheduleMs = ms;
					break;
				case PROF_NETWORK_TRANSMIT:
					BasisEventDriverProfilerData.NetworkTransmitMs = ms;
					break;
				case PROF_JIGGLE_POSE:
					BasisEventDriverProfilerData.JigglePoseMs = ms;
					break;
				case PROF_MICROPHONE:
					BasisEventDriverProfilerData.MicrophoneMs = ms;
					break;
				case PROF_NAMEPLATE_COMPLETE:
					BasisEventDriverProfilerData.NamePlateCompleteMs = ms;
					break;
				case PROF_JIGGLE_COMPLETE_POSE:
					BasisEventDriverProfilerData.JiggleCompletePoseMs = ms;
					break;
				case PROF_SHADOW_CLONE:
					BasisEventDriverProfilerData.ShadowCloneMs = ms;
					break;
			}
		}
		...
		partial void ProfileEnd2(int section)
		{
			...
			switch (section)
			{
				case PROF_NET_TRANSMIT_PICKUPS:
					BasisEventDriverProfilerData.Net_TransmitPickupsMs = ms;
					break;
				case PROF_NET_FIRE_BEFORE_APPLY:
					BasisEventDriverProfilerData.Net_FireBeforeApplyMs = ms;
					break;
				case PROF_NET_SIMULATE_APPLY:
					BasisEventDriverProfilerData.Net_SimulateNetworkApplyMs = ms;
					break;
				case PROF_NET_COMPLETE_REMOTE_LERP:
					BasisEventDriverProfilerData.Net_CompleteRemoteLerpMs = ms;
					break;
				case PROF_NET_MICROPHONE:
					BasisEventDriverProfilerData.MicrophoneMs = ms;
					break;
			}
		}
		...
		partial void ProfileLateUpdateFinish()
		{
			if (!_profiling)
				return;
			_lateUpdateSW.Stop();
			BasisEventDriverProfilerData.LateUpdateTotalMs = _lateUpdateSW
				.Elapsed
				.TotalMilliseconds;
		...
		partial void ProfileBeforeRenderInit()
		{
			...
			if (_profiling)
			{
				BasisEventDriverProfilerData.RemoteFaceJobWasIncomplete =
					BasisRemoteFaceManagement.HasJob
					&& !BasisRemoteFaceManagement.handle.IsCompleted;
		...
		partial void ProfileBeforeRenderFinish()
		{
			if (!_profiling || _beforeRenderSW == null)
				return;
			_beforeRenderSW.Stop();
			BasisEventDriverProfilerData.OnBeforeRenderMs = _beforeRenderSW
				.Elapsed
				.TotalMilliseconds;
		}
#endif
