...
namespace Basis.ImagePickup.Tests
{
	public class BasisImageSecurityTests
	{
		...
		{
			string description = BasisImagePickupRejectionPopup.BuildBatchNotice(
				60,
				10,
				4,
				6
		...
		public void OversizedStaticImageExplainsExceededDimension()
		{
			...
			string path = Path.Combine(
				Path.GetTempPath(),
				$"BasisImagePickup_{Guid.NewGuid():N}.png"
			);
			try
			{
				File.WriteAllBytes(path, pngHeader);
				BasisImageValidationResult result = BasisImageSecurity.ValidateFile(
					path
				);
				...
				Assert.That(
					result.Error,
					Does.Contain("width exceeds the limit by 50px")
				);
				Assert.That(
					result.Error,
					Does.Contain(
		...
		public void OversizedGifUsesAnimationCanvasLimitsBeforeDecode()
		{
			...
			string path = Path.Combine(
				Path.GetTempPath(),
				$"BasisImagePickup_{Guid.NewGuid():N}.gif"
			);
			try
			{
		...
		public void RemoteBudgetAcceptsLargeSmallImageBatch()
		{
			...
			Assert.That(
				...
				Is.GreaterThanOrEqualTo(imageCount)
			);
		}
		...
		public void ValidateFileAcceptsJpegAndReturnsSanitizedPng()
		{
			string path = Path.Combine(
				Path.GetTempPath(),
				$"BasisImagePickup_{Guid.NewGuid():N}.jpg"
			);
			...
			try
			{
				...
				Assert.That(result.CleanPng, Is.Not.Null.And.Not.Empty);
				Assert.That(
					result.CleanPng.Length,
					Is.GreaterThanOrEqualTo(PngSignature.Length)
				);
