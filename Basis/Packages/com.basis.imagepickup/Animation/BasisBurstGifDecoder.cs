...
namespace Basis.ImagePickup
{
	...
	internal sealed class BasisBurstGifDecodeRequest : IDisposable
	{
		...
		public BasisBurstGifDecodeRequest(byte[] source)
		{
			...
			if (source.Length > BasisImagePickupSettings.MaxAnimationSourceBytes)
				throw new ArgumentOutOfRangeException(
					nameof(source),
					"GIF source exceeds the configured limit."
				);
		...
		public BasisBurstGifDecodeRequest(NativeArray<byte> source, bool takeOwnership)
		{
			if (!source.IsCreated || source.Length <= 0)
				throw new ArgumentException(
					"GIF native source is empty.",
					nameof(source)
				);
			if (source.Length > BasisImagePickupSettings.MaxAnimationSourceBytes)
				throw new ArgumentOutOfRangeException(
			...
			try
			{
				...
				else
				{
					...
					NativeArray<byte>.Copy(source, _source, source.Length);
				}
				ScheduleScan();
			}
			catch
		...
		private void ScheduleScan()
		{
			...
			_scanResult = new NativeArray<BasisGifScanResult>(
				1,
				Allocator.Persistent,
		...
		private void Advance(bool block)
		{
			...
			if (_state == RequestState.Scan)
			{
				...
				JobHandle decodeHandle = decodeFramesJob.ScheduleParallelByRef(
					scan.FrameCount,
					1,
					default
				);
				...
				JobHandle clearHandle = fillPosterJob.ScheduleParallelByRef(
					_posterPixels.Length,
					256,
					default
				);
	...
	internal static class BasisBurstGifDecoder
	{
		...
		public static BasisBurstGifDecodeRequest Schedule(
			NativeArray<byte> source,
			bool takeOwnership
		)
		{
			return new BasisBurstGifDecodeRequest(source, takeOwnership);
		}
	...
	internal struct BasisGifScanJob : IJob
	{
		...
		public void Execute()
		{
			...
			while (offset < Source.Length)
			{
				...
				if (
					!TryReadByte(ref offset, out byte minimumCodeSize)
					|| minimumCodeSize < 2
					|| minimumCodeSize > 8
				)
				{
		...
		private bool ReadApplicationSubBlocks(
			ref int offset,
			bool loopExtension,
			ref int rawLoopCount
		)
		{
			bool first = true;
			while (true)
		...
		private static void Fail(
			ref BasisGifScanResult result,
			BasisGifErrorCode error,
			int offset
		)
		{
			if (result.Error != BasisGifErrorCode.None)
	...
	internal struct BasisGifDrawPosterJob : IJobFor
	{
		...
		public void Execute(int index)
		{
			...
			if (source.a == 0)
				return;
			PosterPixels[destination] = BlendStraightOver(
				source,
				PosterPixels[destination]
			);
		}
		...
		)
		{
			return (byte)
				math.min(
					255,
					(source * sw + destination * dw + denominator / 2) / denominator
				);
		}
	}
}
