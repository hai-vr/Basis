...
{
    internal struct ZZHeadBasisAnimationFrameAtlasLocation
    {
    ...
    internal sealed class ZZHeadBasisAnimationFrameAtlas : IDisposable
    {
        ...
        private NativeArray<ZZHeadBasisAnimationFrameAtlasLocation> _locations;
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(ZZHeadBasisAnimationFrameAtlas));
            if (_pages == null || (uint)index >= (uint)_pages.Length)
                throw new ArgumentOutOfRangeException(nameof(index));
            return _pages[index];
        }

        public ZZHeadBasisAnimationFrameAtlasLocation GetLocation(int index) =>
            _locations[index];

        public ZZHeadBasisAnimationFrameAtlas(BasisAnimatedImageData data)
        {
            ...
            try
            {
                _locations = new NativeArray<ZZHeadBasisAnimationFrameAtlasLocation>(
                    frameCount,
                ...
                new ZZHeadBasisAnimationAtlasLayoutJob
                {
                    PageLimit = pageLimit,
        ...
        public bool BeginNextPage(long pixelBudget, out long pixelsUsed)
        {
            ...
            try
            {
                var populateAtlasJob = new ZZHeadBasisAnimationAtlasPopulateJob
                {
                    PageIndex = page,
                    PageWidth = width,
    ...
    [BurstCompile]
    internal struct ZZHeadBasisAnimationAtlasLayoutJob : IJob
    {
        ...
        [WriteOnly]
        public NativeArray<ZZHeadBasisAnimationFrameAtlasLocation> Locations;
        ...
        public void Execute()
        {
            ...
            for (int frameIndex = 0; frameIndex < frameCount; frameIndex++)
            {
                ...
                Locations[frameIndex] = new ZZHeadBasisAnimationFrameAtlasLocation
                {
                    PageIndex = selected,
                    X = sourceX,
                    Y = sourceY,
    ...
    [BurstCompile]
    internal struct ZZHeadBasisAnimationAtlasPopulateJob : IJobFor
    {
        public int PageIndex;
        ...
        {
            ZZHeadBasisAnimationFrameAtlasLocation location = Locations[frameIndex];
