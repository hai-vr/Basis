...
{
    internal enum ZZHeadBasisGifErrorCode
    {
        None = 0,
        BadSignature,
        HeaderTruncated,
    ...
    internal struct ZZHeadBasisGifFrameScan
    {
        public int X;
        public int Y;
        public int Width;
    ...
    internal struct ZZHeadBasisGifScanResult
    {
        public ZZHeadBasisGifErrorCode Error;
    ...
    internal sealed class ZZHeadBasisBurstGifDecodeResult : IDisposable
    {
        public bool Ok;
        public string Error;
    ...
    /// </summary>
    internal sealed class ZZHeadBasisBurstGifDecodeRequest : IBasisAnimationDecodeRequest
    {
        ...
        private NativeArray<ZZHeadBasisGifFrameScan> _scans;
        private NativeArray<ZZHeadBasisGifScanResult> _scanResult;
        ...
        private ZZHeadBasisBurstGifDecodeResult _result;
        ...
        public ZZHeadBasisBurstGifDecodeRequest(byte[] source)
        ...
        internal ZZHeadBasisBurstGifDecodeRequest(NativeArray<byte> source, int length, bool buildPoster, BasisAnimationDecodeTrust trust)
        ...
        private ZZHeadBasisBurstGifDecodeRequest(int length, bool buildPoster, BasisAnimationDecodeTrust trust)
        {
            if (length <= 0)
            ...
            _maxDecodedPixels = ZZHeadBasisBurstGifDecoder.ResolveDecodedPixelLimit(trust);
        ...
        {
            _scans = new NativeArray<ZZHeadBasisGifFrameScan>(
                BasisImagePickupSettings.MaxAnimationFrames,
                Allocator.Persistent,
                NativeArrayOptions.UninitializedMemory
            );
            _scanResult = new NativeArray<ZZHeadBasisGifScanResult>(1, Allocator.Persistent, NativeArrayOptions.ClearMemory);
            _handle = new ZZHeadBasisGifScanJob
            {
                Source = _source,
                Frames = _scans,
                Result = _scanResult,
        ...
        public bool TryComplete(out ZZHeadBasisBurstGifDecodeResult result)
        {
            result = null;
        ...
        public ZZHeadBasisBurstGifDecodeResult Complete()
        {
        ...
        {
            result = null;
            if (!TryComplete(out ZZHeadBasisBurstGifDecodeResult gifResult))
                return false;
        ...
        private BasisBurstAnimationDecodeResult AdaptResult(ZZHeadBasisBurstGifDecodeResult gifResult)
        {
            if (_adapted != null)
                return _adapted;
        ...
        private void Advance(bool block)
        {
            ...
            {
                ZZHeadBasisGifScanResult scan = _scanResult[0];
                if (scan.Error != ZZHeadBasisGifErrorCode.None)
                {
                    FinishFailure(ZZHeadBasisBurstGifDecoder.DescribeError(scan));
                    return;
                }
                ...
                var decodeFramesJob = new ZZHeadBasisGifDecodeFramesJob
                {
                    Source = _source,
                    Scans = _scans,
                    Frames = _frames,
                ...
                if (_buildPoster)
                {
                    var fillPosterJob = new ZZHeadBasisGifFillPosterJob
                    {
                    ...
                    var drawPosterJob = new ZZHeadBasisGifDrawPosterJob
                    {
            ...
            ZZHeadBasisGifScanResult completedScan = _scanResult[0];
            for (int i = 0; i < completedScan.FrameCount; i++)
            {
                if (_decodeErrors[i] == 0)
                    continue;
                var failed = completedScan;
                failed.Error = (ZZHeadBasisGifErrorCode)_decodeErrors[i];
                failed.ErrorFrame = i;
                FinishFailure(ZZHeadBasisBurstGifDecoder.DescribeError(failed));
                return;
            }
            ...
            _pixels = default;
            _frameEnds = default;
            _result = new ZZHeadBasisBurstGifDecodeResult
            {
                Ok = true,
                Animation = animation,
                PosterPixels = _posterPixels,
    ...
    internal static class ZZHeadBasisBurstGifDecoder
    {
        public static ZZHeadBasisBurstGifDecodeRequest Schedule(byte[] source)
        {
            return new ZZHeadBasisBurstGifDecodeRequest(source);
        }
        ...
        )
        {
            ...
            var frames = new NativeArray<ZZHeadBasisGifFrameScan>(
                BasisImagePickupSettings.MaxAnimationFrames,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
            var result = new NativeArray<ZZHeadBasisGifScanResult>(1, Allocator.TempJob, NativeArrayOptions.ClearMemory);
            try
            {
                new ZZHeadBasisGifScanJob
                {
                    Source = window,
                    Frames = frames,
                    Result = result,
                ...
                ZZHeadBasisGifScanResult scan = result[0];
                if (scan.Error != ZZHeadBasisGifErrorCode.None)
                {
        ...
        public static string DescribeError(ZZHeadBasisGifScanResult result)
        {
            string location =
            ...
            {
                ZZHeadBasisGifErrorCode.BadSignature => "Not a GIF (bad signature).",
                ZZHeadBasisGifErrorCode.HeaderTruncated => "GIF header is truncated.",
                ZZHeadBasisGifErrorCode.InvalidCanvas =>
                    "GIF canvas dimensions exceed the configured limits.",
                ZZHeadBasisGifErrorCode.LogicalDescriptorTruncated =>
                    "GIF logical screen descriptor is truncated.",
                ZZHeadBasisGifErrorCode.ColorTableTruncated => "GIF color table is truncated"
                    + location
                    + ".",
                ZZHeadBasisGifErrorCode.ExtensionTruncated => "GIF extension is truncated"
                    + location
                    + ".",
                ZZHeadBasisGifErrorCode.GraphicControlMalformed =>
                    "GIF graphic control extension is malformed" + location + ".",
                ZZHeadBasisGifErrorCode.ApplicationExtensionMalformed =>
                    "GIF application extension is malformed" + location + ".",
                ZZHeadBasisGifErrorCode.SubBlocksTruncated =>
                    "GIF data sub-blocks are truncated" + location + ".",
                ZZHeadBasisGifErrorCode.UnsupportedBlock =>
                    "GIF contains an unsupported block" + location + ".",
                ZZHeadBasisGifErrorCode.TooManyFrames =>
                    $"GIF exceeds {BasisImagePickupSettings.MaxAnimationFrames:N0} frames.",
                ZZHeadBasisGifErrorCode.FrameDescriptorTruncated =>
                    "GIF frame descriptor is truncated" + location + ".",
                ZZHeadBasisGifErrorCode.FrameOutsideCanvas =>
                    "GIF frame lies outside the canvas" + location + ".",
                ZZHeadBasisGifErrorCode.PixelBudgetExceeded =>
                    "GIF exceeds the decoded frame-pixel budget" + location + ".",
                ZZHeadBasisGifErrorCode.MissingColorTable => "GIF frame has no color table"
                    + location
                    + ".",
                ZZHeadBasisGifErrorCode.InvalidCodeSize => "GIF LZW code size is invalid"
                    + location
                    + ".",
                ZZHeadBasisGifErrorCode.NoFrames => "GIF contains no image frames.",
                ZZHeadBasisGifErrorCode.DurationExceeded =>
                    "GIF loop duration exceeds the configured limit" + location + ".",
                ZZHeadBasisGifErrorCode.LzwTruncated => "GIF LZW stream is truncated"
                    + location
                    + ".",
                ZZHeadBasisGifErrorCode.LzwUndefinedCode =>
                    "GIF LZW stream references an undefined code" + location + ".",
                ZZHeadBasisGifErrorCode.LzwCodeOutOfRange => "GIF LZW code is out of range"
                    + location
                    + ".",
                ZZHeadBasisGifErrorCode.LzwDictionaryInvalid =>
                    "GIF LZW dictionary is invalid" + location + ".",
                ZZHeadBasisGifErrorCode.LzwReservedCode =>
                    "GIF LZW dictionary references a reserved code" + location + ".",
                ZZHeadBasisGifErrorCode.LzwOutputOverflow =>
                    "GIF LZW output exceeds the frame size" + location + ".",
                ZZHeadBasisGifErrorCode.LzwOutputSizeMismatch =>
                    "GIF LZW output size does not match the frame" + location + ".",
                ZZHeadBasisGifErrorCode.PaletteIndexOutOfRange =>
    ...
    [BurstCompile]
    internal struct ZZHeadBasisGifScanJob : IJob
    {
        [ReadOnly]
        public NativeArray<byte> Source;

        [WriteOnly]
        public NativeArray<ZZHeadBasisGifFrameScan> Frames;
        public NativeArray<ZZHeadBasisGifScanResult> Result;
        public long MaxDecodedPixels;

        public void Execute()
        {
            var result = new ZZHeadBasisGifScanResult { ErrorFrame = -1 };
            int offset = 0;
            if (Source.Length < 13)
            {
                Fail(ref result, ZZHeadBasisGifErrorCode.HeaderTruncated, offset);
                Result[0] = result;
                return;
            }
            if (
            ...
            {
                Fail(ref result, ZZHeadBasisGifErrorCode.BadSignature, 0);
                Result[0] = result;
                return;
            }
            ...
            if (result.Error != ZZHeadBasisGifErrorCode.None)
            {
                Result[0] = result;
                return;
            }
            ...
            {
                Fail(ref result, ZZHeadBasisGifErrorCode.InvalidCanvas, offset);
                Result[0] = result;
                return;
            }
            ...
            {
                Fail(ref result, ZZHeadBasisGifErrorCode.LogicalDescriptorTruncated, offset);
                Result[0] = result;
                return;
            }
            ...
            if ((logicalPacked & 0x80) != 0)
            {
                ...
                {
                    Fail(ref result, ZZHeadBasisGifErrorCode.ColorTableTruncated, offset);
                    Result[0] = result;
                    return;
                }
            }
            ...
            while (offset < Source.Length)
            {
                ...
                if (blockType == 0x21)
                {
                    ...
                    {
                        Fail(ref result, ZZHeadBasisGifErrorCode.ExtensionTruncated, offset);
                        break;
                    }
                    ...
                    if (extensionLabel == 0xF9)
                    {
                        ...
                        {
                            Fail(ref result, ZZHeadBasisGifErrorCode.GraphicControlMalformed, offset);
                            break;
                        }
                    ...
                    if (extensionLabel == 0xFF)
                    {
                        ...
                        {
                            Fail(ref result, ZZHeadBasisGifErrorCode.ApplicationExtensionMalformed, offset);
                            break;
                        }
                        ...
                        {
                            Fail(ref result, ZZHeadBasisGifErrorCode.SubBlocksTruncated, offset);
                            break;
                        }
                        continue;
                    }
                ...
                if (blockType != 0x2C)
                {
                    Fail(ref result, ZZHeadBasisGifErrorCode.UnsupportedBlock, blockOffset);
                    break;
                }
                if (frameCount >= Frames.Length)
                {
                    result.ErrorFrame = frameCount;
                    Fail(ref result, ZZHeadBasisGifErrorCode.TooManyFrames, blockOffset);
                    break;
                }
                ...
                {
                    result.ErrorFrame = frameCount;
                    Fail(ref result, ZZHeadBasisGifErrorCode.FrameDescriptorTruncated, offset);
                    break;
                }
                if (
                    width <= 0
                ...
                {
                    result.ErrorFrame = frameCount;
                    Fail(ref result, ZZHeadBasisGifErrorCode.FrameOutsideCanvas, offset);
                    break;
                }
                ...
                {
                    result.ErrorFrame = frameCount;
                    Fail(ref result, ZZHeadBasisGifErrorCode.PixelBudgetExceeded, offset);
                    break;
                }
                ...
                {
                    result.ErrorFrame = frameCount;
                    Fail(ref result, ZZHeadBasisGifErrorCode.MissingColorTable, offset);
                    break;
                }
                ...
                {
                    result.ErrorFrame = frameCount;
                    Fail(ref result, ZZHeadBasisGifErrorCode.InvalidCodeSize, offset);
                    break;
                }
                ...
                {
                    result.ErrorFrame = frameCount;
                    Fail(ref result, ZZHeadBasisGifErrorCode.DurationExceeded, offset);
                    break;
                }

                Frames[frameCount] = new ZZHeadBasisGifFrameScan
                {
                    X = left,
            ...
            if (result.Error == ZZHeadBasisGifErrorCode.None && frameCount == 0)
                Fail(ref result, ZZHeadBasisGifErrorCode.NoFrames, offset);
        ...
        private int ReadUInt16(ref int offset, ref ZZHeadBasisGifScanResult result)
        {
        ...
        private static void Fail(ref ZZHeadBasisGifScanResult result, ZZHeadBasisGifErrorCode error, int offset)
        {
            if (result.Error != ZZHeadBasisGifErrorCode.None)
                return;
            result.Error = error;
    ...
    [BurstCompile]
    internal struct ZZHeadBasisGifDecodeFramesJob : IJobFor
    {
        [ReadOnly]
        public NativeArray<byte> Source;

        [ReadOnly]
        public NativeArray<ZZHeadBasisGifFrameScan> Scans;
        ...
        {
            ZZHeadBasisGifFrameScan scan = Scans[frameIndex];
            ...
            var reader = new ZZHeadBasisGifSubBlockBitReader
            {
                Source = Source,
            ...
            while (reader.TryReadCode(codeSize, out int code))
            {
                ...
                if (code == nextCode)
                {
                    if (oldCode < 0)
                    {
                        Errors[frameIndex] = (int)ZZHeadBasisGifErrorCode.LzwUndefinedCode;
                        return;
                    }
                ...
                {
                    Errors[frameIndex] = (int)ZZHeadBasisGifErrorCode.LzwCodeOutOfRange;
                    return;
                }
                ...
                {
                    Errors[frameIndex] = (int)ZZHeadBasisGifErrorCode.LzwReservedCode;
                    return;
                }
                ...
                while (stackCount > 0)
                {
                    ...
                    {
                        Errors[frameIndex] = (int)ZZHeadBasisGifErrorCode.LzwOutputOverflow;
                        return;
                    }
            ...
            {
                Errors[frameIndex] = (int)ZZHeadBasisGifErrorCode.LzwTruncated;
                return;
            }
            if (outputIndex != scan.PixelCount)
                Errors[frameIndex] = (int)ZZHeadBasisGifErrorCode.LzwOutputSizeMismatch;
        }

        private void WritePixel(ZZHeadBasisGifFrameScan scan, int streamIndex, byte paletteIndex)
        {
    ...
    internal struct ZZHeadBasisGifSubBlockBitReader
    {
        [ReadOnly]
        public NativeArray<byte> Source;
        public int Offset;
    ...
    [BurstCompile]
    internal struct ZZHeadBasisGifFillPosterJob : IJobFor
    {
        [WriteOnly]
        public NativeArray<Color32> Pixels;
    ...
    [BurstCompile]
    internal struct ZZHeadBasisGifDrawPosterJob : IJobFor
    {
        public int CanvasWidth;
