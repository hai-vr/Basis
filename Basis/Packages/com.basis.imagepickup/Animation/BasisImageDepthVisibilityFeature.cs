...
namespace Basis.ImagePickup
{
	...
	public sealed class BasisImageDepthVisibilityFeature : ScriptableRendererFeature
	{
		...
		{
			private static readonly int DepthTextureId = Shader.PropertyToID(
				"_BasisImageDepthTexture"
			);
			private static readonly int SampleBufferId = Shader.PropertyToID(
				"_BasisImageDepthSamples"
			);
			private static readonly int VisibilityBufferId = Shader.PropertyToID(
				"_BasisImageDepthVisibility"
			);
			private static readonly int WorldToClipId = Shader.PropertyToID(
				"_BasisImageWorldToClip"
			);
			private static readonly int WorldToViewId = Shader.PropertyToID(
				"_BasisImageWorldToView"
			);
			private static readonly int TextureSizeId = Shader.PropertyToID(
				"_BasisImageDepthTextureSize"
			);
			private static readonly int CardCountId = Shader.PropertyToID(
				"_BasisImageDepthCardCount"
			);
			private static readonly int DepthBiasId = Shader.PropertyToID(
				"_BasisImageDepthBiasMeters"
			);

			private readonly ComputeShader _computeShader;
			private readonly int _kernel;
			private readonly ProfilingSampler _profilingSampler = new(
			...
			)
			{
				...
				TextureDesc depthDescriptor = renderGraph.GetTextureDesc(
					in depthTexture
				);
				...
				BufferHandle visibilityHandle = renderGraph.ImportBuffer(
					visibilityBuffer
				);
				...
				)
				{
					...
					passData.TextureSize = new Vector4(
						width,
						height,
						1f / width,
						1f / height
					...
					builder.SetRenderFunc(
						static (ComputePassData data, ComputeGraphContext context) =>
						{
							...
							context.cmd.SetComputeIntParam(
								data.Shader,
								CardCountId,
								data.CardCount
							);
