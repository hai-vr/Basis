...
namespace Basis.ImagePickup
{
	...
	internal sealed class BasisNativeAnimationPayload : IDisposable
	{
		...
		internal BasisNativeAnimationPayload(
		...
		public void Dispose()
		{
			...
			{
				lock (MemoryBudgetLock)
					_allocatedBytes = Math.Max(
						0,
						_allocatedBytes - _allocatedByteCount
					);
	...
	internal sealed class BasisBurstAnimationEncodeRequest : IDisposable
	{
		...
		public BasisBurstAnimationEncodeRequest(BasisAnimatedImageData data)
		{
			...
			try
			{
				...
				if (
					!BasisAnimatedImageData.TryReserveWorkingBytes(
				...
				JobHandle pixels = pixelPackJob.ScheduleParallelByRef(
					data.PixelsNative.Length,
					512,
					header
				);
				cleanupHandle = pixels;
				var clearHashJob = new BasisLz4ClearHashJob { Hash = _hash };
				JobHandle clearHash = clearHashJob.ScheduleParallelByRef(
					_hash.Length,
					512,
					default
				);
		...
		public BasisBurstAnimationEncodeResult Complete()
		{
			...
			if (
				!BasisNativeAnimationPayload.TryReserveBytes(
					length,
		...
		public void Dispose()
		{
			...
			if (!_resultClaimed)
			{
				_completed?.Payload?.Dispose();
				if (_completed != null)
					_completed.Payload = null;
			}
			_completed = null;
			DisposeNative(true);
		}
	...
	internal sealed class BasisBurstAnimationDecodeRequest : IDisposable
	{
		...
		public BasisBurstAnimationDecodeRequest(
		...
		private void Advance(bool block)
		{
			...
			if (_state == DecodeState.DecompressHeader)
			{
				...
				{
					FinishFailure(
						"Animation native allocation failed: " + exception.Message
					);
					return;
				}
				...
				JobHandle frames = unpackFramesJob.ScheduleParallelByRef(
					header.FrameCount,
					64,
					default
				);
				...
				JobHandle pixels = unpackPixelsJob.ScheduleParallelByRef(
					header.PixelCount,
					512,
					default
				);
				_handle = JobHandle.CombineDependencies(frames, pixels);
				JobHandle.ScheduleBatchedJobs();
			...
			for (int i = 0; i < _errors.Length; i++)
			{
				if (_errors[i] == 0)
					continue;
				FinishFailure(
					BasisBurstAnimationCodec.DescribeError(
						(BasisAnimationCodecError)_errors[i]
					)
				);
				return;
			}
		...
		public void Dispose()
		{
			...
			if (!_resultClaimed)
			{
				_completed?.Animation?.Dispose();
				if (_completed != null)
					_completed.Animation = null;
			}
			_completed = null;
			DisposeAllNative();
		}
	...
	internal static class BasisBurstAnimationCodec
	{
		...
		public static BasisBurstAnimationEncodeRequest ScheduleEncode(
			BasisAnimatedImageData data
		)
		{
			return new BasisBurstAnimationEncodeRequest(data);
		}

		public static BasisBurstAnimationDecodeRequest ScheduleDecode(byte[] payload)
		{
			return new BasisBurstAnimationDecodeRequest(payload);
		}

		public static BasisBurstAnimationDecodeRequest ScheduleDecode(
			NativeArray<byte> payload,
			int length,
			bool takeOwnership
		)
		{
			return new BasisBurstAnimationDecodeRequest(payload, length, takeOwnership);
		}
		...
		public static int CalculateRawLength(BasisAnimatedImageData data)
		{
			...
			if (
				value <= 0
				|| value > BasisImagePickupSettings.MaxAnimationNetworkDecodedBytes
				|| value > int.MaxValue
			)
			{
				throw new InvalidOperationException(
					"Animation raw network body exceeds the configured limit."
				);
			}
			return (int)value;
		}
	...
	internal struct BasisAnimationPackHeaderJob : IJob
	{
		...
		public void Execute()
		{
			...
			WriteInt32(Raw, 12, Frames.Length);
			...
			WriteInt32(
				...
				36,
				BasisBurstAnimationCodec.BodyHeaderBytes
					+ Frames.Length * BasisBurstAnimationCodec.FrameRecordBytes
			);
			...
			for (int frameIndex = 0; frameIndex < Frames.Length; frameIndex++)
			{
				...
				for (
					int reserved = 42;
					reserved < BasisBurstAnimationCodec.FrameRecordBytes;
					reserved++
				)
	...
	internal struct BasisLz4CompressJob : IJob
	{
		...
		public void Execute()
		{
			...
			while (input + 4 <= inputLength)
			{
				...
				while (
					matchLength < maximumMatch
					&& Raw[match + matchLength] == Raw[input + matchLength]
				)
				{
					matchLength++;
				}
		...
		)
		{
			...
			if (
				literalLength >= 15
				&& !WriteExtendedLength(ref output, literalLength - 15)
			)
				return false;
	...
	internal struct BasisLz4DecompressJob : IJob
	{
		...
		public void Execute()
		{
			...
			while (input < inputEnd)
			{
				...
				if (
					literalLength == 15
					&& !ReadExtendedLength(ref input, inputEnd, ref literalLength)
				)
				{
				...
				if (
					input + literalLength > inputEnd
					|| output + literalLength > Raw.Length
				)
				{
					Fail(BasisAnimationCodecError.Truncated);
					return;
				...
				if (
					(token & 15) == 15
					&& !ReadExtendedLength(ref input, inputEnd, ref matchLength)
				)
				{
					Fail(BasisAnimationCodecError.InvalidLz4Token);
					return;
	...
	internal struct BasisAnimationParseBodyHeaderJob : IJob
	{
		...
		public void Execute()
		{
			...
			header.CanvasWidth = ReadInt32(0);
			header.CanvasHeight = ReadInt32(4);
			header.TotalPlayCount = ReadInt32(8);
			header.FrameCount = ReadInt32(12);
			...
			header.TotalDurationMicroseconds = ReadInt64(24);
			header.PixelCount = ReadInt32(32);
			header.PixelDataOffset = ReadInt32(36);
	...
	internal struct BasisAnimationUnpackFramesJob : IJobFor
	{
		...
		public void Execute(int index)
		{
			...
			{
				X = ReadInt32(offset),
				Y = ReadInt32(offset + 4),
				Width = ReadInt32(offset + 8),
				Height = ReadInt32(offset + 12),
				PixelOffset = ReadInt32(offset + 16),
				PixelCount = ReadInt32(offset + 20),
				DurationMicroseconds = ReadInt64(offset + 24),
				EndTimeMicroseconds = ReadInt64(offset + 32),
			...
			if (index > 0)
			{
				...
				long previousEnd = ReadInt64(previousOffset + 32);
				int previousPixelOffset = ReadInt32(previousOffset + 16);
				int previousPixelCount = ReadInt32(previousOffset + 20);
				if (
