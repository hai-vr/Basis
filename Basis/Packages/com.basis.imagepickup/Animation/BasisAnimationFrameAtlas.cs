	...
	internal sealed class BasisAnimationFrameAtlas : IDisposable
	{
		...
		public BasisAnimationFrameAtlas(BasisAnimatedImageData data)
		{
			...
			int pageLimit = math.min(
				PreferredMaximumPageSize,
				math.max(1, SystemInfo.maxTextureSize)
			);
			...
			try
			{
				...
				cursorX = new NativeArray<int>(
					frameCount,
					Allocator.TempJob,
					NativeArrayOptions.ClearMemory
				);
				cursorY = new NativeArray<int>(
					frameCount,
					Allocator.TempJob,
					NativeArrayOptions.ClearMemory
				);
				rowHeight = new NativeArray<int>(
					frameCount,
					Allocator.TempJob,
					NativeArrayOptions.ClearMemory
				);
				usedWidth = new NativeArray<int>(
					frameCount,
					Allocator.TempJob,
					NativeArrayOptions.ClearMemory
				);
				usedHeight = new NativeArray<int>(
					frameCount,
					Allocator.TempJob,
					NativeArrayOptions.ClearMemory
				);
				...
				if (result[2] != 0)
					throw new NotSupportedException(
						"Animation frame does not fit in the device atlas limit."
					);
		...
		public void Dispose()
		{
			...
			{
				if (_pages[i] != null)
					BasisImagePickupRuntimeUtility.DestroyObject(_pages[i]);
			}
			_pages = null;
		}
	}
	...
	internal struct BasisAnimationAtlasLayoutJob : IJob
	{
		...
		public void Execute()
		{
			...
			int pageSize = math.clamp(
				NextPowerOfTwo(math.max(largest, areaExtent)),
				largest,
				PageLimit
			);
			int pageCount = 0;
			for (int frameIndex = 0; frameIndex < Frames.Length; frameIndex++)
			{
				...
				UsedWidth[selected] = math.max(
					UsedWidth[selected],
					placeX + outerWidth
				);
				UsedHeight[selected] = math.max(
					UsedHeight[selected],
					placeY + outerHeight
				);
