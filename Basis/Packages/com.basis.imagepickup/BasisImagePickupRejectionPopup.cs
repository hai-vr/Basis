...
namespace Basis.ImagePickup
{
	internal static class BasisImagePickupRejectionPopup
	{
		...
		{
			ShowDialogue(
				BasisLocalization.Get(RejectionTitleKey),
				BuildDescription(path, reason)
			);
		}
		...
		{
			ShowDialogue(
				BasisLocalization.Get(LimitTitleKey),
				BuildBatchNotice(currentCount, requestedCount, 0, 0)
			);
		}

		public static void ShowBatchNotice(
			int currentCount,
			int requestedCount,
			int allowedCount,
			int animatedCount
		...
		internal static string BuildDescription(string path, string reason)
		{
			...
			return BasisLocalization.Get(
				...
				EscapeRichText(fileName),
				EscapeRichText(reason)
			);
		}
		...
		)
		{
			...
			{
				description.Append(
					BasisLocalization.Get(LimitSummaryKey, limit, currentCount)
				);
			...
			if (animatedCount > BasisImagePickupSettings.AnimationBatchWarningThreshold)
			{
				if (description.Length > 0)
					description.Append("\n\n");
				description.Append(
					BasisLocalization.Get(AnimatedWarningKey, animatedCount)
				);
			}
		...
		private static void ResizeDialogueToContent(
			BasisMenuDialoguePanel dialogue,
			string title
		)
		{
			...
			while (
				DescriptionOverflows(descriptionLabel)
				&& panelData.PanelSize.x + 0.5f < maximumWidth
			)
			{
				panelData.PanelSize.x = Mathf.Min(
					maximumWidth,
					panelData.PanelSize.x + PanelWidthStep
				);
				dialogue.LoadData(panelData);
				RebuildDialogueLayout(dialogue, descriptionLabel);
			}
			...
			if (DescriptionOverflows(descriptionLabel))
			{
				...
				descriptionLabel.fontSizeMin = Mathf.Min(
					originalFontSize,
		...
		)
		{
			...
			return Mathf.Max(
				baseDimension,
				Mathf.Min(viewportMaximum, fallbackMaximum)
			);
		}

		private static void RebuildDialogueLayout(
			BasisMenuDialoguePanel dialogue,
