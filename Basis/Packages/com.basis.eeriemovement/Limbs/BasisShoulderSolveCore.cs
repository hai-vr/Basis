...
namespace Basis.IK
{
    ...
    public static class BasisShoulderSolveCore
    {
        ...
        const float reachEngageThreshold = 0.7f;
        const float setStartDeg = 8f;
        const float setFullDeg = 95f;
        const float trackerRefine = 0.35f;
        const float depressionShare = 0.25f;
        const float depressionBand = 0.12f;
        const float shrugHangStart = 0.75f;
        const float shrugHangFull = 0.92f;
        const float shrugRiseStartFrac = 0.02f;
        const float shrugRiseFullFrac = 0.065f;
        const float shrugBendFadeStartFrac = 0.09f;
        const float shrugBendFadeEndFrac = 0.14f;
