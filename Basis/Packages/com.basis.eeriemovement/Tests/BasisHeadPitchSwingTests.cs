...
namespace Basis.Tests.IK
{
    public class BasisHeadPitchSwingTests
    {
        ...
        {
            BasisHeadPitchSwingCore.Solve(pitchDeg, yawDeg, eyeFromNeck ?? EyeFromNeck, strength, backward,
                out Vector3 offset, out _);
            return offset;
        }
        ...
        {
            Assert.That(Swing(0f).magnitude, Is.LessThan(1e-5f),
        ...
        {
            Assert.That(Swing(-45f).z, Is.LessThan(0f),
        ...
        {
            float down = Swing(50f).z;
            float up = -Swing(-50f).z;
        ...
        public void TheCarry_FollowsTheGazeHeading_NotWorldForward()
        {
            foreach (float yaw in new[] { 0f, 90f, 180f, 270f, 37f })
            {
                ...
                Assert.That(offset.magnitude, Is.EqualTo(Swing(45f, 0f).magnitude).Within(1e-4f),
        ...
        public void TheCarry_NeverTouchesEyeHeight()
        {
            ...
            {
                Assert.That(Swing(pitch).y, Is.EqualTo(0f),
        ...
        public void ScalingTheAvatar_ScalesTheCarryWithIt()
        {
            ...
            Assert.That(Swing(45f, eyeFromNeck: EyeFromNeck * 2f).magnitude, Is.EqualTo(Swing(45f).magnitude * 2f).Within(1e-4f),
        ...
        {
            Assert.That(Swing(45f, 0f, strength: 0f).magnitude, Is.EqualTo(0f),
