...
namespace Basis.Tests.IK
{
    public class BasisArmHitchSlewTests
    {
        ...
        const float k_DragHz = 1.25f;
        const float k_HandSpeedDegPerSec = 90f;
        const int k_HitchAt = 40;
        const int k_Frames = 140;
        ...
        static void RunHand(float[] dts, bool ceiling, out float worstHitchJumpDeg, out float worstSteadyStepDeg)
        {
            ...
            {
                float dt = dts[i];
                az += k_HandSpeedDegPerSec * dt;
                Vector3 hand = k_RightShoulder + Dir(az, -20f) * (0.65f * k_ArmLen);

                if (!BasisSwivelHintCore.ArmHint(frame, k_RightShoulder, hand, k_ArmLen, false,
                                                 out Vector3 modelHint, out float cond))
                {
                    continue;
                }

                Vector3 curAxisV = hand - k_RightShoulder;
                Vector3 rawBendV = modelHint - k_RightShoulder;
                ...
                Vector3 capped = seeded
                    ? (Vector3)BasisElbowSwingCapCore.Apply(prevBend, prevAxis, curAxis, rawBend,
                                                            BasisElbowSwingCapCore.MaxGain,
                                                            curReach - prevReach, cond, slew)
                    : rawBend;

                Vector3 dragged = capped;
                if (seeded)
                {
                    dragged = (Vector3)BasisElbowDragCore.Apply(prevDrag, Quaternion.identity, curAxis, capped,
                                                                BasisElbowDragCore.Alpha(k_DragHz, armDt));

                    float step = Vector3.Angle(prevDrag, dragged);
                    if (i == k_HitchAt) worstHitchJumpDeg = Mathf.Max(worstHitchJumpDeg, step);
        ...
        {
            var dts = new float[k_Frames];
            for (int i = 0; i < k_Frames; i++) dts[i] = 1f / 90f;
            if (hitchMs > 0f) dts[k_HitchAt] = hitchMs / 1000f;
            return dts;
        }
        ...
        {
            var dts = new float[k_Frames];
            for (int i = 0; i < k_Frames; i++) dts[i] = 1f / fps;
            return dts;
        }
        ...
        public void AStalledFrame_CannotFlickTheElbow([Values(50f, 100f, 200f, 333f)] float hitchMs)
        {
            ...
            Assert.Less(jump, allowed,
                $"a {hitchMs:F0} ms stall moved the elbow pole {jump:F1} degrees in ONE frame (ceiling " +
                $"{allowed:F1}). The gain cap is relative to the hand, so a long frame buys a " +
        ...
        public void WithoutTheCeiling_AStallFlicksTheElbow()
        {
            ...
            Assert.Greater(jump, 90f,
                $"the un-ceilinged chain only moved the elbow {jump:F1} degrees on a 333 ms stall -- it " +
                "measured 122 degrees when this gate was written. The defect this test pairs with is gone " +
        ...
        public void TheCeiling_IsInert_AtOrdinaryFramerates([Values(90f, 72f, 45f, 30f)] float fps)
        {
            ...
            Assert.AreEqual(without, withCeiling, 1e-4f,
                $"at {fps:F0} fps the slew ceiling changed the elbow's motion ({withCeiling:F3} vs " +
                $"{without:F3} deg worst step). It is a stutter guard and must be inert wherever the gain " +
                "cap already binds.");
        }
        ...
        public void TheCeiling_KeepsTheBendUnitAndPerpendicular()
        {
            ...
            {
                Vector3 curAxis = new Vector3(
                    (float)(rng.NextDouble() * 2 - 1), (float)(rng.NextDouble() * 2 - 1), (float)(rng.NextDouble() * 2 - 1)).normalized;
                Vector3 prevAxis = (curAxis + 0.3f * new Vector3(
                    (float)(rng.NextDouble() * 2 - 1), (float)(rng.NextDouble() * 2 - 1), (float)(rng.NextDouble() * 2 - 1))).normalized;

                if (!BasisSwivelHintCore.ArmHint(Frame(), k_RightShoulder, k_RightShoulder + curAxis * (0.7f * k_ArmLen),
                                                 k_ArmLen, false, out Vector3 hint, out float cond)) continue;
                Vector3 raw = (hint - k_RightShoulder).normalized;
                Vector3 prev = Vector3.Cross(prevAxis, raw).normalized;
                if (prev.sqrMagnitude < 0.5f) continue;

                Vector3 b = BasisElbowSwingCapCore.Apply(prev, prevAxis, curAxis, raw,
                    BasisElbowSwingCapCore.MaxGain, 0.01f, cond, BasisElbowSwingCapCore.SlewCapRad(1f / 90f));

                Assert.IsTrue(float.IsFinite(b.x) && float.IsFinite(b.y) && float.IsFinite(b.z),
                    $"ceiling went non-finite at axis {curAxis}");
