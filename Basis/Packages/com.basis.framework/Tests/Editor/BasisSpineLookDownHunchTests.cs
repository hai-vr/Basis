    ...
    public class BasisSpineLookDownHunchTests
    {
        ...
        public void ForwardHeadSwing_HunchesTheTorso_AtNoTrackerWeights()
        {
            ...
            for (float gaze = 0f; gaze <= 60f; gaze += 10f)
            {
                ...
                Assert.That(r.SpineEuler.x, Is.GreaterThanOrEqualTo(prevSpine - 1e-3f),
                    $"spine forward bend dropped as the gaze went down ({prevSpine:0.00} -> {r.SpineEuler.x:0.00} at {gaze:0} deg).");
                Assert.That(r.UpperEuler.x, Is.GreaterThanOrEqualTo(prevUpper - 1e-3f),
                    $"upper-chest forward bend dropped as the gaze went down ({prevUpper:0.00} -> {r.UpperEuler.x:0.00} at {gaze:0} deg).");
                prevSpine = r.SpineEuler.x; prevUpper = r.UpperEuler.x;
            }

            var full = Solve(HmdLookDownArc(60f), yaw: 0f, leanWeights: true);
            Assert.That(full.SpineEuler.x, Is.GreaterThan(1.5f),
                $"a forward-swung head produced only {full.SpineEuler.x:0.00} deg of spine hunch; the mechanism under test is gone.");
            Assert.That(full.UpperEuler.x, Is.GreaterThan(0.5f),
                $"a forward-swung head produced only {full.UpperEuler.x:0.00} deg of upper-chest hunch.");
        }
        ...
        public void ChestTrackerPath_RemovesTheLookDownHunch()
        {
            ...
            for (float gaze = 0f; gaze <= 70f; gaze += 10f)
            {
                ...
                Assert.That(Mathf.Abs(r.SpineEuler.x), Is.LessThan(0.05f),
                    $"spine still hunched {r.SpineEuler.x:0.00} deg at {gaze:0} deg gaze with the chest-tracker suppression on.");
                Assert.That(Mathf.Abs(r.UpperEuler.x), Is.LessThan(0.05f),
                    $"upper-chest still hunched {r.UpperEuler.x:0.00} deg at {gaze:0} deg gaze with the chest-tracker suppression on.");
                Assert.That(Mathf.Abs(r.SpineEuler.z), Is.LessThan(0.05f),
                    $"spine lateral bend {r.SpineEuler.z:0.00} deg leaked with the chest-tracker suppression on.");
            }
        }
        ...
        public void ChestTrackerPath_StillFollowsHeadFacingTwist()
        {
            ...
            Assert.That(r.UpperEuler.y, Is.GreaterThan(1f),
                $"head-facing twist was lost ({r.UpperEuler.y:0.00} deg) when the lean pre-bend was suppressed.");
        }
        ...
        public void LateralHeadSwing_RollsTheTorso_AndIsSuppressedWithChestTracker()
        {
            ...
            Assert.That(Mathf.Abs(on.SpineEuler.z), Is.GreaterThan(1f),
                $"a side-swung head did not roll the spine ({on.SpineEuler.z:0.00} deg); the mechanism under test is gone.");

            var off = Solve(sideHead, yaw: 0f, leanWeights: false);
            Assert.That(Mathf.Abs(off.SpineEuler.z), Is.LessThan(0.05f),
                $"spine still rolled {off.SpineEuler.z:0.00} deg with the chest-tracker suppression on.");
            Assert.That(Mathf.Abs(off.UpperEuler.z), Is.LessThan(0.05f),
                $"upper-chest still rolled {off.UpperEuler.z:0.00} deg with the chest-tracker suppression on.");
        }
        ...
        public void ChestTracker_StraightDownAndTurned_NeitherHunchesNorTwists()
        {
            ...
            Assert.That(r.SpineEuler.magnitude, Is.LessThan(0.1f),
                $"spine-bend delta {r.SpineEuler.magnitude:0.000} deg is non-trivial looking straight down with a chest tracker.");
            Assert.That(r.UpperEuler.magnitude, Is.LessThan(0.1f),
                $"upper-chest-bend delta {r.UpperEuler.magnitude:0.000} deg is non-trivial looking straight down with a chest tracker.");
        }
