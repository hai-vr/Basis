...
public static class BasisNetworkGizmoLabelCore
{
    ...
    /// <summary>Change-key for a per-player pose label (interp %, buffer depth, rate, bandwidth).</summary>
    public static int PlayerLabelKey(int playerId, float interpT, float playbackRate, int stagedCount, float bytesPerSecond, float packetsPerSecond, bool showState, bool showBandwidth)
    {
        int k = playerId * 397;
        if (showState)
        {
