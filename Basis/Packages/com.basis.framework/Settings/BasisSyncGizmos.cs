...
public static class BasisSyncGizmos
{
    ...
    public static void Tick(float scale)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            Color health = HealthColor(s);
            bool spatial = Show && s.HasSpatial;
            ...
            {
                // Show off (bandwidth-only) or no spatial channel: a single anchor sphere.
    ...
    private static string BuildLabel(in BasisSyncGizmoSample s, bool showState, bool showBw)
    {
        _text.Clear();
        ...
        if (showBw)
        {
            _text.Append('\n').Append(FormatRate(s.BytesPerSecond)).Append("  ").Append(Mathf.RoundToInt(s.PacketsPerSecond)).Append(" pkt/s");
        }
