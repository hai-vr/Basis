...
namespace Basis.IK.Motion
{
    ...
    ///
    /// Lives in the RUNTIME assembly behind UNITY_EDITOR, like BasisBvhLoader, because both the
    /// NUnit suite (Basis.Framework.IK.Tests) and the editor sweeps (BasisEditor) must reach it and
    /// the tests asmdef does not reference BasisEditor. Putting it editor-side would force the tests
    /// to hand-mirror the maths, and a mirrored metric drifts.
    ///
