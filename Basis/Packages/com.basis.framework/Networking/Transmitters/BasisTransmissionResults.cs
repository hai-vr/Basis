    ...
    public void Simulate()
    {
        ...
        if (AvatarChange)
        {
            for (int index = 0; index < receiverCount; index++)
            {
                ...
                bool newInRange = AvatarRange[index];
                if (remote.InAvatarRange != newInRange)
                {
                    remote.InAvatarRange = newInRange;

                    // Don’t directly “reload”; request a state refresh that only loads if needed.
                    remote.RequestAvatarStateRefresh();
                }
            }
        }
